## this project for Capital of the world game

Country_Captials = {'tirane': 'albania', 'algiers': 'algeria', 'andorra la vella': 'andorra', 'luanda': 'angola',
...
print("========================")
print('CAPITALS OF THE WORLDS')
print("========================")

start = 1
while start ==1: 
    capital= input ("Please Enter any Capital in the world:  ").lower()
    if capital in Country_Captials: 
        start = 0 
    else :
        print(f"{capital} is not the capital")


score = 1
game = 1
while game == 1: 
    print (f"correct , {capital.title() } is the capital of {Country_Captials[capital].title()}")
    last_letter = capital[-1]
    capital = input (f"What is the capital in the world starts with '{last_letter.upper()}' letter? ").lower()
    if capital [0] == last_letter and capital in Country_Captials : 
        score += 1 
    else : 
        game = 0  
        for correct_capital in Country_Captials:
            if correct_capital[0] == last_letter:
                print (f"wrong , {capital} is not correct , you can say {correct_capital.title()} the capital of {Country_Captials[correct_capital].title()} ")
                break
